...
function CompactPostCard({ post }: { post: Post }) {
  ...
  return (
    <Link href={`/blog/${post.metadata.slug}`} className="block h-full">
      <Card className="flex flex-col h-full hover:bg-muted/50 transition-colors border-border/50 shadow-sm hover:shadow-md cursor-pointer overflow-hidden p-4 group">
    ...
    <Link href={`/blog/${post.metadata.slug}`} className="block h-full">
      <Card className="h-full flex flex-col hover:bg-muted/50 transition-colors border-border/50 shadow-sm hover:shadow-md cursor-pointer overflow-hidden group relative">
        ...
        <div className="p-6 flex flex-col flex-1">
          ...
          <h3 className="font-bold text-2xl line-clamp-2 mb-4 group-hover:text-primary transition-colors">
            {post.metadata.title}
          </h3>
